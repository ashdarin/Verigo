...
el("list-add-to-button")?.addEventListener("click", async () => { const id = window.location.pathname.split("/").pop(); if (!id || !state.listSelection.size) return; try { const lists = await api("/api/lists"); const choices = lists.filter((list) => list.id !== id); if (!choices.length) { el("list-detail-meta").textContent = VerigoI18n.text("还没有其他列表"); return; } const target = choices[0]; await api(`/api/lists/${encodeURIComponent(target.id)}/results`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ result_ids:[...state.listSelection], added_from:"cross-list" }) }); el("list-detail-meta").textContent = `${VerigoI18n.text("已加入")} ${target.name}`; } catch (error) { el("list-detail-meta").textContent = error.message; } });
