const VerigoI18n = (() => {
  const english = {
    ...
    "没有符合条件的结果": "No results match the current filters", "正在等待首条验证结果": "Waiting for the first verification result", "下载失败": "Download failed", "未验证": "Not verified", "正在生成验证结果": "Generating verification results", "任务名称（可选）": "Job name (optional)", "导出候选 CSV": "Export candidate CSV",
    "正在确认，请稍候。": "Checking. Please wait.", "本次验证未完成，请稍后再次验证。": "This check did not finish. Please try again later.", "验证已停止，尚未形成结论。": "Verification stopped before a conclusion was reached.", "可以联系。首次发送建议小批量进行。": "This address can be contacted. Start with a small first send.", "暂不建议发送，等待对方清理收件箱后再联系。": "Do not send yet. Try again after the recipient clears their inbox.", "请检查域名拼写后再联系。": "Check the domain spelling before trying again.", "请确认该域名是否用于接收邮件。": "Confirm whether this domain receives email.", "建议更换或人工确认该联系地址。": "Use a different address or confirm this one manually.", "暂时无法确认，建议稍后再次验证。": "Unable to confirm yet. Please try again later.",
    "正在验证中...": "Verification in progress...", "验证失败，请重新验证": "Verification failed. Try again.", "验证已取消": "Verification canceled.", "有效邮箱，可直接使用": "Valid email. Ready to use.", "收件箱已满，暂时无法接收邮件": "Inbox is full and cannot receive email right now.", "域名不存在，请检查拼写": "Domain does not exist. Check the spelling.", "该域名未配置邮件服务": "This domain is not configured to receive email.", "无效邮箱，建议删除": "Invalid email. Remove it from your list.", "状态未知，建议稍后重试": "Status unknown. Try again later.",
    "名字（拼音）": "First name (Pinyin)", "姓氏（拼音）": "Last name (Pinyin)", "三步找到有效的工作邮箱": "Find a usable work email in three steps", "输入真实姓名和公司域名后，Verigo 会生成候选地址。提交验证后，再根据状态决定是否继续联系。": "Enter a real name and company domain. Verigo generates candidate addresses, then you can decide whether to contact them based on the result.", "1. 输入信息": "1. Enter details", "姓名和公司域名越准确，候选结果越相关。": "More accurate names and domains produce more relevant candidates.", "2. 验证候选": "2. Verify candidates", "确认候选地址是否适合联系。": "Confirm whether each candidate address is suitable for contact.", "3. 导出结果": "3. Export results", "下载 CSV，按状态安排后续跟进。": "Download a CSV and plan follow-up by status.", "候选地址": "Candidate addresses", "根据姓名与域名生成可能的工作邮箱。": "Generate possible work emails from a name and domain.", "验证状态": "Verification status", "查看可投递、不可投递或待确认结果。": "See deliverable, undeliverable, or inconclusive results.", "结果解释": "Result explanation", "每条结果提供原因与后续建议。": "Each result includes a reason and recommended next step.",
